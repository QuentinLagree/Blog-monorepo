...
describe('InputErrorComponent', () => {
  ...
  it("Should display an error message if fields value is less than minLength", () => {
    ...
    const compiled = fixture.nativeElement as HTMLElement;
    expect(compiled.textContent?.trim()).toBe(`Le texte doit contenir au moins ${minLength} caractère${minLength == 1 ? '' : 's'} (actuellement ${control.value?.length}).`);
  })
  ...
  it("Should display an error message if fields value is less than maxLength", () => {
    ...
    const compiled = fixture.nativeElement as HTMLElement;
    expect(compiled.textContent?.trim()).toBe(`Le texte ne doit pas dépasser ${maxLength} caractère${maxLength == 1 ? '' : 's'} (actuellement ${control.value?.length}).`);
  })

  it("Should display an error message if fields are an error unexcepted", () => {
    const control = new FormControl('abcabcbcabcabc', {validators: [
      () => ({ unexceptedError: true})
    ]});
    control.markAsTouched();
    component.control = control;
    fixture.detectChanges();
