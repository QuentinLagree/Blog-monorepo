...
export class ToastService {
  ...
  success(message: string, opts: Omit<ToastOptions, 'message' | 'variant'> = {}) {
    return this.show({ ...opts, message, variant: 'success', title: 'Succès' });
  }
  error(message: string, opts: Omit<ToastOptions, 'message' | 'variant'> = {}) {
    return this.show({ ...opts, message, variant: 'error', title: 'Erreur' });
  }
  info(message: string, opts: Omit<ToastOptions, 'message' | 'variant'> = {}) {
    return this.show({ ...opts, message, variant: 'info', title: 'Infos' });
  }
  warning(message: string, opts: Omit<ToastOptions, 'message' | 'variant'> = {}) {
    return this.show({ ...opts, message, variant: 'warning', title: 'Attention !' });
  }
  ...
  show(options: ToastOptions) {
    ...
    if (
      this.config.deduplicate &&
      this.visible.some((t) => t.message === toast.message && t.variant === toast.variant)
    ) {
      return toast.id;
    }
  ...
  private startTimer(t: ToastInternal) {
    ...
    const handle = setTimeout(() => this.close(t.id), remaining);
  ...
  private durationFor(variant: ToastVariant) {
    switch (variant) {
      case 'error':
        return Math.max(5000, this.config.defaultDuration);
      case 'warning':
        return this.config.defaultDuration + 1000;
      default:
